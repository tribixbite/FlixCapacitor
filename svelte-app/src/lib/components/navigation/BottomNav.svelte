<script lang="ts">
  ...
  import { favoritesCount } from '$stores/favorites.store';
  import { activeDownloads, queuedDownloads } from '$stores/downloads.store';
  import { derived } from 'svelte/store';

  const { impact } = useHaptics();

  // Derived store for active download count (active + queued, not completed)
      ...
      >
        <div class="nav-icon-wrapper">
          ...
          {#if item.id === 'favorites' && $favoritesCount > 0}
            <span class="nav-badge">{$favoritesCount > 99 ? '99+' : $favoritesCount}</span>
          {/if}
          <!-- Badge for downloads (active + queued only) -->
          {#if item.id === 'downloads' && $activeDownloadCount > 0}
            <span class="nav-badge">{$activeDownloadCount > 99 ? '99+' : $activeDownloadCount}</span>
          {/if}
        </div>
