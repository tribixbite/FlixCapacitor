...
function showAddTorrentDialog(): void {
    ...
    // Get elements
    const input = document.getElementById('torrent-url-input') as HTMLInputElement;
    const fileButton = document.getElementById('select-torrent-file') as HTMLButtonElement;
 ...
 * Handle magnet link
 */
function handleMagnetLink(magnetURL: string): void {
    console.log('Adding magnet link:', magnetURL);

    const Settings = (window as any).Settings;
    if (Settings) {
        Settings.droppedMagnet = magnetURL;
    }

    const NativeTorrentClient = (window as any).NativeTorrentClient;
    const App = (window as any).App;

    // Mobile app: Use NativeTorrentClient directly instead of old provider system
    if (NativeTorrentClient && App && App.UI) {
        try {
        ...
        (window as any)._pendingMagnetLink = magnetURL;
        ...
        setTimeout(() => {
            const pendingURL = (window as any)._pendingMagnetLink;
            if (pendingURL) {
                delete (window as any)._pendingMagnetLink;
                handleMagnetLink(pendingURL);
            }
        }, 2000);
    }
}
 ...
 * Handle torrent URL
 */
function handleTorrentURL(torrentURL: string): void {
 ...
 * Handle torrent file
 */
function handleTorrentFile(file: File): void {
    ...
    reader.onload = function (e) {
        ...
        const blob = new Blob([arrayBuffer as ArrayBuffer], { type: 'application/x-bittorrent' });
