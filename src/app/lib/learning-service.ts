...
class LearningService {
    ...
    async fetchCoursesCSV(): Promise<string> {
        ...
        try {
            ...
            const response = await fetch('/data/academic-torrents-courses.csv');

            if (response.ok) {
                const csvData = await response.text();
                console.log('✅ Local CSV loaded successfully, size:', csvData.length, 'bytes');
                return csvData;
            } else {
        ...
        // Try streaming server proxy as fallback
        try {
            ...
            console.log('Attempting to fetch via streaming server proxy:', proxyUrl);
        ...
        // Try direct fetch as fallback (will likely fail due to CORS)
        try {
            console.log('Attempting direct fetch from Academic Torrents...');
        ...
        console.log('⚠️ All fetch methods failed, using embedded demo data fallback');
