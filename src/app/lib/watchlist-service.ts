...
import type { ContentType, FavoriteItem, FavoritesQueryOptions } from './favorites-service';

// Reuse types from favorites-service since they're identical
export type WatchlistItem = FavoriteItem;
export type WatchlistQueryOptions = FavoritesQueryOptions;
...
class WatchlistService {
   ...
   */
  async addToWatchlist(item: WatchlistItem): Promise<boolean> {
    try {
   ...
   */
  async getWatchlist(options: WatchlistQueryOptions = {}): Promise<WatchlistItem[]> {
    try {
      const {
        type = null,
        limit = 100,
      ...
      const results = await this.db.all(query, params) as WatchlistRow[];
   ...
   */
  private detectType(item: WatchlistItem): ContentType {
