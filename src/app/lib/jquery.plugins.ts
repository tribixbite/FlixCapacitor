...
(function ($: any) {
    ...
    $.fn.drags = function () {
        const $el = this;
        const win = (window as any).win || window;
        return $el.css('cursor', 'move').on('mousedown', function (e: any) {
            const $drag = $(this).addClass('draggable');
            ...
            const max_y = win.height - 0;

            $drag.css('z-index', 1000).parents().on('mousemove', function (e: any) {
                if (e.pageX < 0 || e.pageX > win.width || e.pageY < 0 || e.pageY > win.height) {
                    $(this).off('mousemove');
                    $('.draggable').css('z-index', z_idx).removeClass('draggable');
                } else {
                    const fin_y = e.pageY + pos_y - drg_h;
